...
func (lpht *quadraticProbingHashTable) dump() {
...
func (lpht *quadraticProbingHashTable) find(name string) (int, int) {
	hash := hash(name) % lpht.capacity
	deletedIndex := -1
	for i := 0; i < lpht.capacity; i++ {
		index := (hash + i*i) % lpht.capacity
...
func (lpht *quadraticProbingHashTable) set(name string, phone string) {
...
func (lpht *quadraticProbingHashTable) get(name string) string {
	key, _ := lpht.find(name)
...
func (lpht *quadraticProbingHashTable) contains(name string) bool {
	key, _ := lpht.find(name)

	return key > -1 && lpht.employees[key] != nil && !lpht.employees[key].deleted
}

func (lpht *quadraticProbingHashTable) delete(name string) {
	key, _ := lpht.find(name)
...
func (lpht *quadraticProbingHashTable) probe(name string) int {
	// Hash the key.
	hash := hash(name) % lpht.capacity
...
func (lpht *quadraticProbingHashTable) dumpConcise() {
...
func (lpht *quadraticProbingHashTable) aveProbeSequenceLength() float32 {
	totalLength := 0
	numValues := 0
