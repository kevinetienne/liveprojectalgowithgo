...
func newLinearProbingHashTable(capacity int) *doubleHashTable {
...
func (lpht *doubleHashTable) dump() {
...
func (lpht *doubleHashTable) find(name string) (int, int) {
	h1 := hash1(name) % lpht.capacity
	h2 := hash2(name) % lpht.capacity
	deletedIndex := -1
	for i := 0; i < lpht.capacity; i++ {
		index := (h1 + i*h2) % lpht.capacity
...
func (lpht *doubleHashTable) set(name string, phone string) {
...
func (lpht *doubleHashTable) get(name string) string {
	key, _ := lpht.find(name)
...
func (lpht *doubleHashTable) contains(name string) bool {
	key, _ := lpht.find(name)

	return key > -1 && lpht.employees[key] != nil && !lpht.employees[key].deleted
}

func (lpht *doubleHashTable) delete(name string) {
	key, _ := lpht.find(name)
...
func (lpht *doubleHashTable) probe(name string) int {
	// Hash the key.
	h1 := hash1(name) % lpht.capacity
	h2 := hash2(name) % lpht.capacity
...
func (lpht *doubleHashTable) dumpConcise() {
...
func (lpht *doubleHashTable) aveProbeSequenceLength() float32 {
	totalLength := 0
	numValues := 0
